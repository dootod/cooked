...
export async function generateMetadata({
  ...
  return {
    title: `#${formatTagName(slug)} — Cooked`,
    description: `Recettes avec le tag ${formatTagName(slug)}`,
  };
}
...
export default async function TagDetailPage({
  ...
  const { recipes, total } = await getRecipesByTag(slug, page);
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[#0a0f1e]">
          <div
            className="absolute top-[30%] left-[20%] w-[350px] h-[350px] bg-accent/[0.08] rounded-full blur-[150px]"
            style={{ animation: "public-glow-pulse 8s ease-in-out infinite" }}
          />
          <div
            className="absolute bottom-[20%] right-[10%] w-[300px] h-[300px] bg-primary/[0.06] rounded-full blur-[120px]"
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16 sm:pb-20">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20 mb-4">
            <span className="text-sm font-medium text-accent">#{tagName}</span>
          </div>
